local internalNpcName = "Bruno Forge Masters"
...
npcConfig.voices = {
    interval = 15000,
    chance = 50,
    { text = "Ola, aventureiro! Precisa de armas ou armaduras? Eu sou o cara!" },
}
...
npcType.onSay = function(npc, creature, type, message)
    ...
    -- Verifique se a mensagem do jogador é "shop"
    if message:lower() == "shop" then
        local player = Player(creature)
        local playerId = player:getId()

        -- Abra a janela de compra
        npcHandler:shop(cid)
    end
end
...
local function creatureSayCallback(npc, creature, type, message)
    -- Não faz nada aqui, pois todas as outras interações foram removidas
    return false
end

keywordHandler:addKeyword({ "trade" }, StdModule.say, { npcHandler = npcHandler, text = "Eu sou o ferreiro dos Deuses. Se voce precisar de armas ou armaduras, basta me perguntar." })

npcHandler:setMessage(MESSAGE_GREET, "Bem-vindo a minha loja, aventureiro |PLAYERNAME|! Diga {trade} se quiser armas e armaduras.")
npcHandler:setMessage(MESSAGE_FAREWELL, "Adeus e volte novamente, |PLAYERNAME|.")
npcHandler:setMessage(MESSAGE_WALKAWAY, "Adeus e volte novamente.")
